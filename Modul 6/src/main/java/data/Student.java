...
public class Student extends User implements iMenu {
    ...
    public static void showBorrowedBooks(){
        ...
        gridPane.add(backButton, 0, 1); // menambahkan tombol Back ke GridPane
    ...
    public static void returnBooks(){
        ...
        ObservableList<Book> borrowedBooks = FXCollections.observableArrayList(Book.arr_borrowedBook);
        ...
        submitButton.setOnAction(event -> {
            ...
            for (Book borrowedBook : Book.arr_borrowedBook) {
                if (borrowedBook.getBookId().equals(bookIdField.getText())) {
                    for (Book book : Book.arr_bookList) {
                        if (book.getBookId().equals(bookIdField.getText())) {
                            ...
                            break;  // Exit inner loop, no need to check other books
                        }
                    }

                    if (validasiReturnBooks) {
                        borrowedBooks.remove(borrowedBook);  // Remove from ObservableList
                        break;  // Exit outer loop, no need to check other borrowed books
                    }
                }
            }
